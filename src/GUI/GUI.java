...
public class GUI {
    ...
    private static final String[][] FILTERS =
            {{"Ôàéëû Word (*.docx)", "*.docx"}, {"Ôàéëû Excel (*.xlsx)", "*.xlsx"},
                    {"Ôàéëû Adobe (*.pdf)", "*.pdf"}, {"Âñå ôàéëû (*.*)", "*.*"}};
    ...
    public static void main(String[] arguments) {
        ...
        canvas.addPaintListener(new PaintListener() {
            public void paintControl(PaintEvent e) {
                        ...
                        switch (a[i][j]) {
                            ...
                            case ('@'):
                                if (j == gameState.getHead().x && i == gameState.getHead().y)
                                    drawColoredSq(e, j * sqRez, i * sqRez, sqRez - 1,
                ...
                e.gc.drawText("Äëèíà çìåéêè:  " + gameState.getLenght(), 10,
                        sqRez * (a.length));
                if (flag) {
            ...
            public void keyPressed(KeyEvent e) {
                switch (e.keyCode) {
                    ...
                    case (SWT.F4):
                        display.timerExec(-1, gameTick);
                        flag = false;
                        runGame(canvas, ".\\Snake\\levels\\FirstOne.txt", gameTick);
                        break;
                    case (SWT.F2):
        ...
        runGame(canvas, ".\\levels\\Small.txt", gameTick);
